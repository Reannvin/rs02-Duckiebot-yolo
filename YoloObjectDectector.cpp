...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          std::cout << "Detected the  "<<classLabels_[i] << '\n'<<std::endl;
          boundingBox.id = i;
          ...
          std::cout<<"Detect the  "<<boundingBox.Class<<std::endl;

          if (i == 0) {
            std::cout<<"Go ahead."<<std::endl;
            teleop_.linear.x = 0.10;
            vel_pub.publish(teleop_);
          }
          else if (i ==1) {
            std::cout<<"Left"<<std::endl;
            teleop_.angular.z = -0.05;
            vel_pub.publish(teleop_);
          }
          else if (i ==2) {
            ...
            teleop_.angular.y=0;
            teleop_.angular.z=0;
            std::cout<<"send message that teleop_ stop all"<<std::endl;
            vel_pub.publish(teleop_);

            std::cout<<"Now stop the node."<<std::endl;
            ros::shutdown();
          }
          else if (i==4) {
            std::cout<<"Obstacle"<<std::endl;
            if 
            teleop_.angular.z = 0.0;
            teleop_.linear.x=0.0;
            vel_pub.publish(teleop_);
          }
          else{
            std::cout<<"No singal detected."<<std::endl;
            //teleop_.linear.x = 0.0;
            std::cout<<"Do not send any message to cmd_vel"<<std::endl;
            vel_pub.publish(teleop_);
          }
    ...
    vel_pub = nodeHandle_.advertise<geometry_msgs::Twist>("/cmd_vel",1);
    std::cout<<"Nothing happend. Cool"<<std::endl;
...
teleop_.angular.y=0;
teleop_.angular.z=0;
std::cout<<"HOW MANY OBJECT DO WE GOT?!"<<std::endl;
std::cout<<numClasses_<<"     da!!!"<<std::endl;
vel_pub.publish(teleop_);
*/
